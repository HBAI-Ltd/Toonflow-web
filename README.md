...
近期前端重点围绕“生产工作台可用性、素材节点操作、移动端构建适配”进行了同步：

- **生产画布节点布局优化**  
  修复流程图节点卡片重叠问题，调整 `flowBuilder` 与生产页节点渲染逻辑，让剧本、分镜、素材等节点在复杂项目中更容易扫描和操作。
- **角色场景与素材管理增强**  
  更新场景/角色资产页，补充素材删除、加入资产库、批量生成后的状态同步等交互，配合后端新增接口完成资产闭环。
- **分镜与剧本面板体验调整**  
  优化分镜表格、剧本节点、ScriptAgent 计划节点等面板的显示与操作反馈，减少长内容编辑和批量生成时的跳动感。
- **图像工具与生成状态提示更新**  
  图像工具、生产状态 Store、生成任务展示补充失败原因与任务状态，便于用户判断模型返回、网络异常或参数配置问题。
- **多语言文案同步**  
  更新简体中文与英文文案，覆盖近期新增的素材操作、生成流程和错误提示。
- **Vite 构建配置补充**  
  新增 `vite.config.js` 与类型声明，方便桌面端、Web 端和移动端复用构建配置。
